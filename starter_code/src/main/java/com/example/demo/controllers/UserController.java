...
public class UserController {
	...
	public ResponseEntity<User> createUser(@RequestBody CreateUserRequest createUserRequest) {
		...
		if (createUserRequest.getPassword().length() < 7 ||
			...
			log.error("Password is either less than 7 characters or is not the same as confirm password. Please enter the correct password and try again.", createUserRequest.getUsername());
			return ResponseEntity.badRequest().build();
		}
		User byUsername = userRepository.findByUsername(createUserRequest.getUsername());
		if (byUsername != null) {
			log.warn("Username \"{}\" already exists. Please choose a different username and try again", createUserRequest.getUsername());
			return ResponseEntity.badRequest().build();
		}
